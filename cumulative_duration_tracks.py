...
top_artists = spotify_df.groupby("track_title")["listening_duration_seconds"].sum().sort_values(ascending=False).head(TOP_X).index.tolist()

df_top = spotify_df[spotify_df["track_title"].isin(top_artists)].copy()

df_top["listen_time_hours"] = df_top["listening_duration_seconds"]/3600

pivot = (
    df_top.pivot_table(index="timestamp", columns="track_title", values="listen_time_hours", aggfunc="sum")
    .fillna(0)
...
plt.tight_layout()
plt.savefig(f"./results/cumulative_duration_top_{TOP_X}_tracks.png", dpi=150)
print(f"Result saved in results/cumulative_duration_top_{TOP_X}_tracks.png")
# plt.show()